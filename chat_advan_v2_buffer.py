    ...
    async def completion_generator(request: fastapi.Request) -> StreamingResponse:
        try:
            ...
            async def generate_text():
                ...
                buffer = ""  # Add a buffer to accumulate text
                ...
                async for result in engine.generate(prompt, sampling_params, request_id):
                    if len(result.outputs) > 0:
                        ...
                        # Only yield the new content since last yield
                        if len(new_text) > last_yielded_position:
                            new_part = new_text[last_yielded_position:]
                            buffer += new_part
                            
                            # Find complete words and punctuation
                            words = buffer.split()
                            if len(words) > 1:  # If we have at least 2 words
                                ...
                                buffer = words[-1]
                                yield to_yield + ' '  # Add space after each yield
                ...
                if buffer:
                    # Apply same spacing rules to final buffer
                    for punct in ['.', '!', '?']:
